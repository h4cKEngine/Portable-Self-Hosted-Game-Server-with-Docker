...
4. Verranno creati sul tuo Desktop i collegamenti con le rispettive icone:
   - `StartMcServer.lnk` (icona Minecraft 2D logo) per avviare il server e la dashboard.
   - `InstallServer.lnk` (icona Minecraft cartella) per aggiornare la repository o reinstallare.
   Il file `.bat` iniziale scaricato sul Desktop viene sostituito automaticamente dai collegamenti con icona!

> **Nota Aggiornamento**: Facendo doppio clic su `InstallServer.lnk`, scarichera' automaticamente l'ultima versione del server da GitHub, funzionando cosi' come un comodo tasto per aggiornare la repository!
