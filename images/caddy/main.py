...
SERVER_IPS_PATH = ENV_DIR_PATH / "server_ips.env"
SERVER_IPS_EXAMPLE_PATH = ENV_DIR_PATH / "server_ips.env-example"

def parse_server_ips() -> dict:
    target = None
    if SERVER_IPS_PATH.is_file():
        target = SERVER_IPS_PATH
    elif SERVER_IPS_EXAMPLE_PATH.is_file():
        target = SERVER_IPS_EXAMPLE_PATH

    data = {"ip_server": "127.0.0.1", "ip_vpn1": "", "ip_vpn2": ""}
    if target:
        parsed = parse_env_file(target)
        data["ip_server"] = parsed.get("IP_SERVER", "127.0.0.1").strip() or "127.0.0.1"
        data["ip_vpn1"] = parsed.get("IP_VPN1", "").strip()
        data["ip_vpn2"] = parsed.get("IP_VPN2", "").strip()
    elif ENV_FILE_PATH.is_file():
...
def get_server_info():
    """Returns dynamic server information including full DDNS FQDN and configured server IPs."""
    ...
    return {
        "ip": ips.get("ip_server") or parsed.get("IP_SERVER", os.getenv("IP_SERVER", "127.0.0.1")),
        "ips": ips,
        "domain": full_domain,
...
def get_config():
    ...
    config_data = {
        ...
        "ddns_skip": parsed.get("DDNS_SKIP", "false").lower() == "true" or ((OVERRIDES_DIR_PATH / "ddns.skip").exists() if OVERRIDES_DIR_PATH.exists() else False),
...
# ─── Server IPs Management Endpoints ─────────────────────────────────────────

class ServerIpsRequest(BaseModel):
    ip_server: Optional[str] = "127.0.0.1"
    ip_vpn1: Optional[str] = ""
    ip_vpn2: Optional[str] = ""

@app.get("/api/server-ips")
def get_server_ips():
    """Returns the configured server IPs from server_ips.env."""
    return {
        "status": "success",
        "data": parse_server_ips()
    }

@app.post("/api/server-ips")
def save_server_ips(req: ServerIpsRequest):
    """Saves the 3 configured server IPs to env/server_ips.env."""
    try:
        ip_server = (req.ip_server or "").strip() or "127.0.0.1"
        ip_vpn1 = (req.ip_vpn1 or "").strip()
        ip_vpn2 = (req.ip_vpn2 or "").strip()

        content = [
            "# Server IP Configurations",
            "# Specificare fino a 3 indirizzi IP per il server (Normale, VPN 1, VPN 2)",
            f"IP_SERVER={ip_server}",
            f"IP_VPN1={ip_vpn1}",
            f"IP_VPN2={ip_vpn2}",
            ""
        ]
        ...
        ENV_DIR_PATH.mkdir(parents=True, exist_ok=True)
        with open(SERVER_IPS_PATH, "w", encoding="utf-8") as f:
            f.write("\n".join(content))

        return {
            "status": "success",
            "message": "Indirizzi IP salvati con successo in env/server_ips.env.",
            "data": {
                "ip_server": ip_server,
                "ip_vpn1": ip_vpn1,
                "ip_vpn2": ip_vpn2
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save server IPs: {str(e)}")
